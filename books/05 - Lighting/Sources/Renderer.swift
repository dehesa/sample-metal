  ...
  struct Uniforms {
    ...
    init(_ previous: Self?) {
      if let previous {
        ...
        self.rotation.x = duration * (.𝝉 / 4.0)
        self.rotation.y = duration * (.𝝉 / 6.0)
    ...
    func matrices(size: CGSize) -> (modelView: float4x4, projection: float4x4, normal: float3x3) {
      ...
      let projectionMatrix = float4x4(perspectiveWithAspect: Float(size.width)/Float(size.height), fovy: .𝝉/5, near: 0.1, far: 100)
