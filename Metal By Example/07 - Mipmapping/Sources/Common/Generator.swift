...
enum Generator {
  ...
  enum Texture {
    ...
    static func makeSimpleCheckerboard(size: CGSize, tileCount: Int, pixelFormat: MTLPixelFormat, with metal: (device: MTLDevice, queue: MTLCommandQueue)) throws -> MTLTexture {
      ...
      (try _makeCheckerboardImage(size: size, tileCount: tileCount)).data.withUnsafeBytes { (ptr) in
    ...
    static func makeTintedCheckerboard(size: CGSize, tileCount: Int, pixelFormat: MTLPixelFormat, with device: MTLDevice) throws -> MTLTexture {
      ...
      let (data, image) = try _makeCheckerboardImage(size: size, tileCount: tileCount)
      ...
      while mipWidth > 1 && mipHeight > 1 {
        ...
        let tintColor = _makeTintColor(level: level - 1)

        let scaled = try _makeResizeImage(image: levelImage, size: CGSize(width: mipWidth, height: mipHeight), tintColor: tintColor)
...
private extension Generator.Texture {
  /// Returns a tintColor depending on the mip level.
  static func _makeTintColor(level: Int) -> CGColor {
    switch level {
  ...
  static func _makeCheckerboardImage(size: CGSize, tileCount: Int) throws -> (data: Data, image: CGImage) {
  ...
  static func _makeResizeImage(image: CGImage, size: CGSize, tintColor: CGColor) throws -> (data: Data, image: CGImage) {
    let (width, height) = (Int(size.width), Int(size.height))
