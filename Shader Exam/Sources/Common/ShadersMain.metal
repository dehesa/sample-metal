...
fragment half4 fragment_main(VertexOut in [[stage_in]], texture2d<float, access::sample> diffuseTexture [[texture(0)]]) {
    constexpr sampler sampler2d(coord::normalized, filter::linear);
    
    const float4 color = diffuseTexture.sample(sampler2d, in.texCoords);
